...
export const VideoDownloader = () => {
  ...
  const handleDownload = async (sourceUrl = url) => {
    ...
    try {
      const result = await downloadMedia(info.canonicalUrl || sourceUrl, info.platform, controller.signal);
      ...
      } else if (result.error === 'Recherche annulée') {
  ...
  const handleShare = async () => {
    const shareUrl = buildShareUrl(window.location.href, linkInfo?.canonicalUrl || url);
    if (!shareUrl) {
      setError('Impossible de créer un lien de partage pour cette URL.');
      return;
    }
    try {
      ...
      setError('Impossible de copier le lien de partage.');
    }
  };
  ...
  const handleImportHistory = async (file: File | undefined) => {
    ...
    try {
      ...
      const next = importHistoryJson(text);
      setHistory(next);
      if (next.length === 0) {
        setError('Aucun lien valide dans le fichier importé.');
      }
    } catch {
      setError('Impossible d’importer cet historique.');
    }
  };

  const handleClearHistory = () => {
    if (history.length === 0) return;
    if (!window.confirm('Vider tout l’historique local ?')) return;
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                aria-invalid={Boolean(linkInfo && !linkInfo.isValid)}
              />
              {linkInfo && (
          ...
          {isDownloading ? (
            <button
              ...
              className="w-full py-4 px-6 rounded-xl bg-gray-700 text-white font-semibold text-base sm:text-lg hover:bg-gray-800 transition-all"
            >
          ...
          {error && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 space-y-3" role="alert">
              ...
              {linkInfo?.isValid && !isDownloading && (
        ...
        {history.length > 0 && (
          <section className="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3" aria-label="Historique des recherches">
            ...
            <input
              ...
              className="hidden"
              aria-label="Importer un historique JSON"
              onChange={(event) => {
                const file = event.target.files?.[0];
                void handleImportHistory(file);
                event.target.value = '';
              }}
            />
            {history.length > 3 && (
